...
public class V104__PermissionsForTransfers extends LiquibaseJavaMigration {

    @Override
    public Integer getChecksum() {
        return Checksum.V104__PermissionsForTransfers;
    }

    @Override
    protected void doMigrate(Context context, DataSource dataSource, NamedParameterJdbcTemplate named, JdbcDataContext dataContext) throws Exception {
        Table m_permission = dataContext.getDefaultSchema().getTableByName("m_permission");
        insert_m_permission(named, m_permission, "portfolio_group", "TRANSFERCLIENTS_GROUP", "GROUP", "TRANSFERCLIENTS", 0);
        insert_m_permission(named, m_permission, "portfolio_group", "TRANSFERCLIENTS_GROUP_CHECKER", "GROUP", "TRANSFERCLIENTS", 0);
        insert_m_permission(named, m_permission, "portfolio", "TRANSFER_CLIENT", "CLIENT", "TRANSFER", 0);
        insert_m_permission(named, m_permission, "portfolio", "TRANSFER_CLIENT_CHECKER", "CLIENT", "TRANSFER", 0);
    }
