...
import com.angkorteam.fintech.client.dto.PostHookRequest;
import com.angkorteam.fintech.meta.tenant.MHookSchema;
...
@Bookmark("/admin/system/hook/create")
public class HookCreatePage extends MasterPage {
    ...
    protected void onInitData() {
        ...
        try (DataSet rows = dataContext.query().from(mHookTemplate).select(mHookTemplate.NAME).where(mHookTemplate.ID).eq(this.templateId).execute()) {
            rows.next();
            this.templateValue = (String) rows.getRow().getValue(mHookTemplate.NAME);
        }
    ...
    protected void onInitHtml(MarkupContainer body) {
        ...
        this.form2.add(this.createButton);
    ...
    protected void createButtonSubmit() {
        ...
        PostHookRequest request = new PostHookRequest();
        ...
        for (Map<String, Object> event : this.configurationBrowseValue) {
            request.getEvents().add(new PostHookRequest.Event((String) event.get("entity_name"), (String) event.get("action_name")));
        }
        client.hookCreate(getSession().getIdentifier(), getSession().getToken(), request);
