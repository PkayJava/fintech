...
public class ExternalIdValidator implements IValidator<String> {
    ...
    public void validate(IValidatable<String> validatable) {
        ...
        if (externalId != null && !"".equals(externalId)) {
            ...
            long count = 0;
            if (this.id == null) {
                try (DataSet rows = dataContext.query().from(mFund).select(FunctionType.COUNT, mFund.ID).where(mFund.EXTERNAL_ID).eq(externalId).execute()) {
                    rows.next();
                    count = (long) rows.getRow().getValue(0);
                }
            } else {
                try (DataSet rows = dataContext.query().from(mFund).select(FunctionType.COUNT, mFund.ID).where(mFund.EXTERNAL_ID).eq(externalId).and(mFund.ID).ne(this.id).execute()) {
                    rows.next();
                    count = (long) rows.getRow().getValue(0);
                }
            }
